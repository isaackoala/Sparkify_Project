...
#### Version Modified
修改了项目定义中的第三个要求“清晰定义了能衡量模型或结果的性能的评价指标。根据问题的特点调整了评价指标。”

尽管上一版中也定义了F1得分作为模型评价指标，但未给出了基于本项目数据特点的解释。

本版去除了准确率，只选择F1得分作为模型性能评价指标，并对基于数据特点的选择做出解释。
...
此外除了准确率外，还有像精确率和召回率可用以评价分类模型优劣。然而，像目前这样的以发生顾客流失为阳性的数据集。尤其顾客流失量非常少，可能会造成精确率变得很低，而召回率变得很高，同时各种预选模型的值可能都会很接近，这样就失去了评价指标本身的区别模型优劣的意义了。 因此也无法在本项目中使用。

另外如ROC曲线，受试者工作特征曲线，虽然也广泛用于分类模型评价。但其以假阳性率（False Positive Rate, FPR）为横坐标，以真阳性率（True Positive Rate, TPR）为纵坐标的本质已经决定它对于阳性值极少的不平衡数据集，很难给出有效的模型优劣的评定。其效果往往没有F1得分那么敏感。

有鉴于此，本项目使用F1得分作为模型的评价指标。
